...
# Determine static folder path (only used if frontend is co-located; not present on Render)
...
def load_models():
    """Load all pre-trained ML artifacts from the models/ directory."""
    ...
    logger.info("Loading pre-trained models from: %s", models_dir)
    for artifact in required_artifacts:
        path = os.path.join(models_dir, artifact)
        if not os.path.exists(path):
            raise FileNotFoundError(
                f"Missing required artifact: {artifact} at {path}"
            )
...
try:
    ...
    _models_error = str(e)
    logger.critical("FATAL — Failed to load models: %s", _models_error)
...
def index():
    """Home endpoint — serves HTML if browser requests it, otherwise API metadata."""
    accept = request.headers.get("Accept", "")
    static_index = os.path.join(app.static_folder, "index.html") if app.static_folder else None
    if "text/html" in accept and static_index and os.path.exists(static_index):
...
def health():
    """Health check endpoint — returns 200 when models are loaded, 503 otherwise."""
    if not _models_loaded:
        return jsonify({
            "status": "unhealthy",
            "error": _models_error or "Models not loaded",
        }), 503
...
def predict():
    """Main prediction endpoint — accepts user profile JSON, returns full recommendation."""

    # Guard: fail fast with a clean 503 if models aren't ready
    if not _models_loaded:
        return jsonify({
            "error": "Service unavailable — models failed to load",
            "details": _models_error or "Unknown startup error",
        }), 503

    try:
        data = request.get_json(force=True, silent=True)
        if not data:
            return jsonify({"error": "No JSON body provided or Content-Type incorrect"}), 400

        # ── 1. Field presence validation ────────────────────────────────────
        required_fields = [
            "name", "age", "gender", "height", "weight",
            "sleep", "steps", "workout_hours", "goal", "diet_pref", "medical",
        ]
        ...
        if diet_pref not in ["Balanced", "High Protein", "Low Carb", "Keto", "Vegetarian"]:
            return jsonify({"error": "Diet preference must be one of: Balanced, High Protein, Low Carb, Keto, Vegetarian"}), 400
        if medical not in ["None", "Diabetes", "Hypertension", "Asthma"]:
        ...
        # ── 5. BMR + Activity-factor calorie estimate ───────────────────────
        ...
        # ── 6. ML Model 1: Calorie Prediction (Linear Regression) ──────────
        ...
        X_user_cal = pd.DataFrame(
            [[age, gender_enc, height, weight, round(bmi, 2),
              workout_hours, sleep, steps, goal_enc]],
            columns=cols_cal,
        )
        ml_calories = int(calorie_model.predict(X_user_cal)[0])

        # ── 7. ML Model 2: Workout Recommendation (Random Forest) ──────────
        ...
        X_user_wk = pd.DataFrame(
            [[age, gender_enc, round(bmi, 2), workout_hours, steps, goal_enc]],
            columns=cols_wk,
        )
        ...
        # ── 8. ML Model 3: Diet Recommendation (Cosine Similarity) ─────────
        ...
        # ── 10. LSTM Weight Progress simulation ─────────────────────────────
        ...
        predicted_s    = float(np.mean(scaled_history[-3:]) * 0.98)
        ...
        logger.error("Error during /predict: %s", str(e), exc_info=True)
        return jsonify({
